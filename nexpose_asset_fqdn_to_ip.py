...
def ip_from_fqdn(fqdn):
	api_response = search_assets(field='host-name',operator='is',lower=None,upper=None,value=fqdn,values=None)
	#print(api_response)
	try:
...
if args.single:
	response = ip_from_fqdn(args.single)
	print(f"{response['fqdn']},{response['ip']}")
